# SUPERSTES Technical Design Document

Build spec. Companion to `superstes-script.md` (all narrative content, the source of truth),
`superstes-art-prompts.md` (art direction and asset filenames), and `superstes-sound-design.md` (audio
...
/superstes
  ...
  /css
    style.css
  /js
    i18n.js        // I18N — locale registry, ?lang= switch, UI_EN
    scenes.js      // SCENES_C1 — Chapter I scene data
    endings.js     // ENDINGS_C1 + WITNESS_CALLBACK_C1
    wall.js        // WALL_C1 — Chapter I tally wall copy
    scenes-c2.js   // SCENES_C2 — Chapter II scene data
    endings-c2.js  // ENDINGS_C2 + DEPARTURE_CALLBACK
    wall-c2.js     // WALL_C2 — Chapter II tally wall copy
    content-zh.js  // GENERATED Chinese content pack; see superstes-localization.md
    chapters.js    // chapter registry; points SCENES/ENDINGS/WALL at one chapter
  ...
  /assets
    /images        // art assets (Ch I + Ch II), filenames per the art docs
      /thumbs      // 400px wall tier, generated by scripts/optimize_images.sh
    /sound         // audio assets, filenames per superstes-sound-design.md
    /widgets       // standalone HTML prototypes; not loaded by the game
  /content
    /zh            // hand-edited JSON masters for the Chinese pack
```

Load order in `index.html`: `i18n.js` → every chapter's data files → the translation packs →
`chapters.js` → `audio.js` → `gallery.js` → `main.js`. See § "Two chapters, one page" below for
why the chapter data comes before `chapters.js`, and § "Two languages, one page" for the rest.

`audio.js` must load before `main.js` (which calls into `Sound`) and may read `SCENES` for bed
preloading, so it goes after the data files and `chapters.js`. It owns every audio filename and every
decision about when a sound plays; `main.js` only reports events to the `Sound` facade and holds no
audio state. See [superstes-sound-design.md](superstes-sound-design.md) §12 for that interface.
...
wall's own copy (heading, register names, credit) lives in each chapter's `wall*.js`. See §7 for its
behavior.
...
`/content` holds the hand-editable JSON masters for each translation; `js/content-<lang>.js` is
generated from them by `scripts/build_content.py` and committed, because the game cannot `fetch`
JSON over `file://`. Never hand-edit a generated pack. See
[superstes-localization.md](superstes-localization.md).

Asset filenames are defined in the art prompts docs and must match exactly. Do not invent new names
...
`superstes-chapter-2-script.md` needs **no schema additions**. It is simpler than Chapter I: no
...
Localization reuses the same seam and adds nothing to the engine's shape. A translation is one
generated file that registers a full set of chapter data with `I18N`; a chapter's four getters in
`chapters.js` go through `I18N.content(name, fallback)`, so the same pointer swap now selects
...
All of these are backward-compatible — Chapter I's behavior is unchanged:
...
| `gallery.js` tolerates a wall missing `credit`, `secretPrompt`, or `again` | Optional fields. Chapter II's wall has no plaque and no `nextChapter`, but it does carry `again` (script doc, reversed decision). A wall without `again` would render no exit at all. |
| Wall layout keyed by register `layout` role | Chapter II renames every register; label-keyed CSS would collapse its grid. |
| `Gallery.reset()` clears catalogue cache and `revealAll` | Chapter switch must not reuse the previous chapter's catalogue or a `?all` ink. |
